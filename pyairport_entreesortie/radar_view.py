...
from PyQt5.QtCore import QCoreApplication, QPointF, QRectF, Qt, pyqtSignal
from PyQt5.QtGui import QBrush, QColor, QKeySequence, QPainterPath, QPen, QPolygonF
...
BUILDING_COLOR = "lightgreen" # Terminals and other buildings

# Brushes
STAND_BRUSH = QBrush(QColor(STAND_COLOR))
POINT_BRUSH = QBrush(QColor(POINT_COLOR))
BUILDING_BRUSH = QBrush(QColor(BUILDING_COLOR))
...
class RadarView(QWidget):
    ...
    def create_airport_view(self):
        ...
        # Buildings
        if hasattr(self.simulation.airport, 'terminals') and self.simulation.airport.terminals:
            for building in self.simulation.airport.terminals:
                pt_list=[QPointF(pt.x,pt.y) for pt in building.coordinates]
                qpoly = QPolygonF(pt_list)
                item = QGraphicsPolygonItem(qpoly, airport_group)
                item.setPen(pen)
                item.setBrush(BUILDING_BRUSH)
