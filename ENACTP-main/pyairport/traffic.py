...
class Flight:
    ...
    def distance(self,other,t):
        return abs(self.get_position(t)-other.get_position(t))
    def conflicts(self,other,t):
        return self.distance(other,t)<70
# Load a traffic file
'''
...
def detect(flights,t):
    s=set()
    n=len(flights)-1
    for (i,flg) in enumerate(flights):
        if i==n:
            break
        for other in flights[(i+1):]:
            if flg.conflicts(other,t):
                s.add(flg)
                s.add(other)
    return s
...
def detect_in(flights,t1,t2):
    ...
    for j in range(t1,t2):
        for (i,flg) in enumerate(flights):
            ...
            if flg.end_t > j:
                for other in flights[(i+1):]:
                    if other.end_t > j and flg.conflicts(other,j):
                        s.add(flg)
                        s.add(other)
    return s
